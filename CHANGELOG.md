...
- **CI / tooling:** validation workflow uses short parallel leaf jobs (`features`, `fixtures`, `errors`, `format`, `lint`, `types`, `test`) with aggregator `validate`; pre-commit hook names aligned; removed `make check`, `make quality`, and `make test-cov` aliases; `make format` is check-only (rewrite via pre-commit); coverage gate 100% in `pyproject.toml`.
